...
public class UserService {
    ...
    @Value("${user.profile.img.default}")   // 처음 유저가 IoC에 생성될 때, value 값을 autowired 처럼 쓰는 것.
    ...
    private User checkUsernameAndPassword(String username, String password) {
        ...
        if(user == null) {
            throw new UsernameNotFoundException("사용자 정보를 다시 확인하세요.");   // id 체크
        }

        if(!passwordEncoder.matches(password, user.getPassword())) {
            throw new BadCredentialsException("사용자 정보를 다시 확인하세요.");     // 비밀번호 체크
        }
    ...
    public OAuth2User joinSignin(ReqOAuth2SignupDto dto) {
        User user = checkUsernameAndPassword(dto.getUsername(), dto.getPassword());
        return OAuth2User.builder()
                .userId(user.getId())
                .oAuth2Name(dto.getOauth2Name())
